...
function permiso($nivel=1, $usuario_id=NULL, $salir=TRUE, $mensaje=TRUE) {

   global $gcm, $_SESSION ;

   $nivel_de_usuario_actual = $gcm->au->nivel();
   $usuario_id_actual       = $gcm->au->id();

   if ( $usuario_id ) {

      if ( $usuario_id_actual == $usuario_id && $nivel_de_usuario_actual >= $nivel) {
         return TRUE;
      } elseif ( $usuario_id_actual != $usuario_id && $nivel_de_usuario_actual > $nivel) {
         return TRUE;
      } else {
         $aviso = literal('No tenemos suficientes permisos para gestionar a otros usuarios');
         }

   } else {
      
      if ( $gcm->au->nivel() >= $nivel ) {
         return TRUE;
      } else {
         if ( $mensaje ) registrar(__FILE__,__LINE__,'Se necesitan permisos más elevados para esta acción','ERROR');
         }

   }

   if ( $salir  ) {
      registrar(__FILE__,__LINE__,'Se necesitan permisos más elevados para esta acción','ERROR');
...
function rmdir_recursivo($directorio) {
   ...
   if (rmdir($directorio) ) {
      ...
      registrar(__FILE__,__LINE__,"Directorio $directorio eliminado");

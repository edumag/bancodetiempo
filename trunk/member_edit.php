...
if($_REQUEST["mode"] == "admin") {  //         Administrator is editing a member's account
	...
	$form->addElement("header", null, l("Editar Asociat")." " . $member->person[0]->first_name . " " . $member->person[0]->last_name);
...
$menu_imagen[] = &HTML_QuickForm::createElement('button','check',l('Canviar'),'onClick=window.location="capture_photo.php?member_id='.$member->member_id.'"'); 
$form->addGroup($menu_imagen,'imagen','Foto',null,null);
	...
	$form->addElement("header", null, l("Editar la meva informació personal"));
	$form->addElement("html", "<TR></TR>");
	$form->addElement("hidden","member_id", $cUser->member_id);
	$form->addElement("hidden","mode","self");
	$update_text=l("Amb quina freqüència vols rebre correu-e?");
}	
...
$form->addElement("static", null, null, null);
$form->addElement('submit', 'btnSubmit', l('Guardar el canvis'), array('class' => 'formulari2'));

 if($_REQUEST["mode"] == "admin") {  // Administrator is editing a member's account
    $cUser->MustBeLevel(1);
    $form->addElement('submit', 'botonFicha', l('Generar carnet'), array('class' => 'formulari2'));
 } 
//
// Define form rules
//
        ...
        if(isset($_REQUEST['botonFicha'])) 
            {
                ...
                $ficha->GenerarFicha();
                $list .= l("Fitxa generada.");
            }
        else if(isset($_REQUEST['btnSubmit'])) 
            $list .= l("Canvis realitzats.");
	} else {
